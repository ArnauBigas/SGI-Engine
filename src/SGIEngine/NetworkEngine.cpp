...
bool NetworkEngine::init(bool client) {
...
bool NetworkEngine::connectToServer(const char* ip, int port) {
    if (isServer) {
...
void NetworkEngine::acceptIncomingConnections(bool accept) {
    acceptIncoming = accept;
}

bool NetworkEngine::checkInboundConnections() {
    //Server started
